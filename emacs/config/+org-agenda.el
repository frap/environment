...
(defconst +agenda--stuck-projects
  '(tags-todo "PROJECT-ANNULÉ-SUSPENDUÉ/!"
...
(defconst +agenda--project-subtasks
  '(tags-todo "-REFILE-ANNULÉ-ATTENDRE-SUSPENDUÉ/!"
...
(defconst +agenda--waiting
  '(tags-todo "-ANNULÉ+ATTENDRE-READING-FOCUS|+SUSPENDUÉ/!"
              ((org-agenda-overriding-header
                (concat "Waiting y  Postponed Taches"
                        (if +agenda-hide-scheduled-and-waiting-next-tasks
                            ""
                          " (including WAITING and SCHEDULED tasks)")))
...
(defun +agenda/toggle-next-task-display ()
  ...
  (when  (equal major-mode 'org-agenda-mode)
    (org-agenda-redo))
  (message "%s ATTTENDRE y SUIVANT Tâches Programm" (if +agenda-hide-scheduled-and-waiting-next-tasks "Hide" "Show")))
    ...
    (let ((next-headline (save-excursion (or (outline-next-heading) (point-max)))))
      (if (+agenda--is-project-p)
          (let* ((subtree-end (save-excursion (org-end-of-subtree t)))
            ...
            (save-excursion
              (forward-line 1)
              (while (and (not has-next) (< (point) subtree-end) (re-search-forward "^\\*+ SUIV " subtree-end t))
    ...
    (let* ((subtree-end (save-excursion (org-end-of-subtree t)))
      ...
      (cond
       ...
       ((and limit-to-project
             (+agenda--is-project-subtree-p)
             (member (org-get-todo-state) (list "SUIV")))
        subtree-end)
       (t
        nil)))))
